...
void MEXTI_voidInit (void)
{
	...
	{
		/*Set the EXTI port*/
		SYSCFG->EXTICR[((MEXTI_Config[local_u8Iterator].Port)/4)] &= ~(FOUR_BIT_MASK << (((MEXTI_Config[local_u8Iterator].Port)%4)*4));
		SYSCFG->EXTICR[((MEXTI_Config[local_u8Iterator].Port)/4)] |=  (MEXTI_Config[local_u8Iterator].Port << (((MEXTI_Config[local_u8Iterator].Port)%4)*4));
		/*Set the EXTI mode*/
